...
public class lectures_db extends SQLiteOpenHelper {
    static final private String DB_NAME="LECTURE_PLANS";
    static final private String DB_TABLE="LECTURES";
    static final private int DB_Version=1;

    Context ctx;
    SQLiteDatabase my_DB;
    public lectures_db(@Nullable Context context) {
...
//            String s3= (String) DateFormat.format("hh:mm aa",cal2);
            arrPlans.add(new lectures_model(id,title,desc,cal));
        }
        my_DB.close();
        cr.close();
        return arrPlans;
